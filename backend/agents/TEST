import pandas as pd
import numpy as np
from scipy import stats
from scipy.stats import chi2_contingency, f_oneway, shapiro
from typing import Dict, Any, Optional
from llama_index.core.tools import FunctionTool
from llama_index.core.agent import FunctionCallingAgent
from llama_index.llms.openai import OpenAI
from . import AgentBase

class StatAgent(AgentBase):
    name = "stat"

    def __init__(self, llm_api_key: Optional[str] = None, base_url: Optional[str] = None):
        self.llm_api_key = llm_api_key
        self.base_url = base_url
        self.llm = None
        if llm_api_key:
            self.llm = OpenAI(
                api_key=llm_api_key,
                base_url=base_url if base_url else "https://api.deepseek.com/v1",
                model="deepseek-chat"
            )

    def run_ttest(self, df: pd.DataFrame, num_col: str, cat_col: str) -> Dict[str, Any]:
        try:
            groups = df[cat_col].dropna().unique()
            if len(groups) < 2:
                return {"error": f"Need at least 2 groups in {cat_col}, found {len(groups)}"}
            
            group_counts = df[cat_col].value_counts()
            top_groups = group_counts.head(2).index.tolist()
            
            x = df[df[cat_col] == top_groups[0]][num_col].dropna()
            y = df[df[cat_col] == top_groups[1]][num_col].dropna()
            
            if len(x) < 3 or len(y) < 3:
                return {"error": f"Not enough data: {len(x)} vs {len(y)} samples"}
            
            t_stat, p_value = stats.ttest_ind(x, y, equal_var=False, nan_policy="omit")
            
            return {
                "test": "independent_t_test",
                "numeric_column": num_col,
                "group_column": cat_col,
                "groups": [str(top_groups[0]), str(top_groups[1])],
                "n1": int(len(x)),
                "n2": int(len(y)),
                "t_statistic": round(float(t_stat), 4),
                "p_value": round(float(p_value), 6),
                "interpretation": "significant" if p_value < 0.05 else "not significant"
            }
        except Exception as e:
            return {"error": str(e)}

    def run_chi_square(self, df: pd.DataFrame, cat_col1: str, cat_col2: str) -> Dict[str, Any]:
        try:
            contingency_table = pd.crosstab(df[cat_col1], df[cat_col2])
            if contingency_table.size == 0 or (contingency_table == 0).all().all():
                return {"error": "No data in contingency table"}
            
            chi2, p_value, dof, expected = chi2_contingency(contingency_table)
            
            return {
                "test": "chi_square_test",
                "column1": cat_col1,
                "column2": cat_col2,
                "chi2_statistic": round(float(chi2), 4),
                "p_value": round(float(p_value), 6),
                "degrees_of_freedom": int(dof),
                "interpretation": "associated" if p_value < 0.05 else "independent"
            }
        except Exception as e:
            return {"error": str(e)}

    def run_anova(self, df: pd.DataFrame, num_col: str, cat_col: str) -> Dict[str, Any]:
        try:
            groups = df[cat_col].dropna().unique()
            if len(groups) < 2:
                return {"error": f"Need at least 2 groups in {cat_col}, found {len(groups)}"}
            
            group_data = []
            for group in groups:
                gv = df[df[cat_col] == group][num_col].dropna()
                if len(gv) >= 2:
                    group_data.append(gv)
            
            if len(group_data) < 2:
                return {"error": "Not enough groups with sufficient data"}
            
            f_stat, p_value = f_oneway(*group_data)
            
            return {
                "test": "anova",
                "numeric_column": num_col,
                "group_column": cat_col,
                "groups_count": len(groups),
                "f_statistic": round(float(f_stat), 4),
                "p_value": round(float(p_value), 6),
                "interpretation": "significant differences" if p_value < 0.05 else "no significant differences"
            }
        except Exception as e:
            return {"error": str(e)}

    def run_shapiro_wilk(self, df: pd.DataFrame, num_col: str) -> Dict[str, Any]:
        try:
            data = df[num_col].dropna()
            if len(data) < 3:
                return {"error": f"Need at least 3 samples, got {len(data)}"}
            if len(data) > 5000:
                data = data.sample(5000, random_state=42)
            
            stat, p_value = shapiro(data)
            
            return {
                "test": "shapiro_wilk",
                "column": num_col,
                "statistic": round(float(stat), 4),
                "p_value": round(float(p_value), 6),
                "interpretation": "normal" if p_value > 0.05 else "not normal",
                "sample_size": len(data)
            }
        except Exception as e:
            return {"error": str(e)}

    def run_all_statistical_tests(self, df: pd.DataFrame) -> Dict[str, Any]:
        results = {}
        
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        categorical_cols = df.select_dtypes(exclude=[np.number]).columns.tolist()
        
        # Normality
        results["normality_tests"] = {}
        for col in numeric_cols:
            results["normality_tests"][col] = self.run_shapiro_wilk(df, col)
        
        # T-tests
        results["t_tests"] = []
        for num_col in numeric_cols[:3]:
            for cat_col in categorical_cols[:2]:
                if len(df[cat_col].dropna().unique()) >= 2:
                    r = self.run_ttest(df, num_col, cat_col)
                    r["combination"] = f"{num_col} ~ {cat_col}"
                    results["t_tests"].append(r)

        # ANOVA
        results["anova_tests"] = []
        for num_col in numeric_cols[:2]:
            for cat_col in categorical_cols[:2]:
                if len(df[cat_col].dropna().unique()) >= 2:
                    r = self.run_anova(df, num_col, cat_col)
                    r["combination"] = f"{num_col} ~ {cat_col}"
                    results["anova_tests"].append(r)

        # Chi-square
        results["chi_square_tests"] = []
        for i, c1 in enumerate(categorical_cols[:3]):
            for c2 in categorical_cols[i+1:i+3]:
                r = self.run_chi_square(df, c1, c2)
                r["combination"] = f"{c1} × {c2}"
                results["chi_square_tests"].append(r)

        # Pearson correlation
        if len(numeric_cols) >= 2:
            corr_matrix = df[numeric_cols].corr().round(3)
            results["pearson_correlation_matrix"] = corr_matrix.to_dict()

            mask = np.triu(np.ones(corr_matrix.shape), k=1).astype(bool)
            corr_upper = corr_matrix.where(mask)
            flat = corr_upper.stack().abs().sort_values(ascending=False)

            results["top_correlations"] = [
                {
                    "col1": c1,
                    "col2": c2,
                    "corr_abs": float(val),
                    "corr": float(corr_matrix.loc[c1, c2])
                }
                for (c1, c2), val in flat.head(5).items()
            ]
        else:
            results["pearson_correlation_matrix"] = {}
            results["top_correlations"] = []

        return results

    def generate_statistical_summary(self, test_results: Dict[str, Any]) -> str:
        if not self.llm:
            return self._generate_fallback_summary(test_results)
        try:
            tools = [
                FunctionTool.from_defaults(
                    fn=lambda: test_results,
                    name="get_statistical_results",
                    description="Get complete statistical test results"
                )
            ]
            
            agent = FunctionCallingAgent.from_tools(
                tools, 
                llm=self.llm, 
                system_prompt=(
                    "Ты — статистик-аналитик. Проанализируй результаты статистических тестов и создай краткий "
                    "отчет на русском языке."
                )
            )
            
            return str(agent.chat("Сформируй отчет."))
        except Exception:
            return self._generate_fallback_summary(test_results)

    def _generate_fallback_summary(self, test_results: Dict[str, Any]) -> str:
        summary = ["СТАТИСТИЧЕСКИЙ АНАЛИЗ ДАННЫХ\n"]
        
        normal = []
        not_normal = []
        for col, r in test_results.get("normality_tests", {}).items():
            if "interpretation" in r:
                (normal if r["interpretation"]=="normal" else not_normal).append(col)

        if normal:
            summary.append(f"Нормальное распределение: {', '.join(normal)}")
        if not_normal:
            summary.append(f"Ненормальное распределение: {', '.join(not_normal)}")

        sig_t = [
            f"{t['combination']} (p={t['p_value']})"
            for t in test_results.get("t_tests", [])
            if t.get("p_value", 1) < 0.05
        ]
        if sig_t:
            summary.append(f"Значимые различия (t-test): {', '.join(sig_t)}")

        sig_a = [
            f"{t['combination']} (p={t['p_value']})"
            for t in test_results.get("anova_tests", [])
            if t.get("p_value", 1) < 0.05
        ]
        if sig_a:
            summary.append(f"Значимые различия (ANOVA): {', '.join(sig_a)}")

        strong_corr = [
            f"{c['col1']}–{c['col2']} (r={c['corr']})"
            for c in test_results.get("top_correlations", [])
            if abs(c["corr"]) > 0.7
        ]
        if strong_corr:
            summary.append(f"Сильные корреляции: {', '.join(strong_corr)}")

        return "\n".join(summary)

    def run(self, ctx: dict) -> dict:
        dataset_path = ctx["files"]["dataset"]

        if dataset_path.endswith(".csv"):
            df = pd.read_csv(dataset_path)
        else:
            df = pd.read_excel(dataset_path)

        test_results = self.run_all_statistical_tests(df)
        summary = self.generate_statistical_summary(test_results)

        ctx.setdefault("metrics", {})
        ctx["metrics"]["statistical_tests"] = test_results

        ctx.setdefault("insights", [])
        ctx["insights"].append(summary)

        ctx["brief"] = "Statistical analysis completed"

        self.save_context(ctx)
        return ctx